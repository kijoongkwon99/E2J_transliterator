...
class MoraeKanaConverter:
    ...
    def convertMorae_orig(self, morae):
        sounds = morae.split('.')
    ...
    def convertMorae(self, morae):
        ...
        # first mora
        first = sounds[0]
        kana = self.mora_map.get(first)

        if kana is None:
            warnings.warn(f"Unknown mora skipped: {first}")
        else:
            result += kana
        ...
        for idx in range(1, len(sounds)):
            ...
            # long vowel
            if cur in self.vowels and prev and prev[-1] == cur:
                result += self.doubled

            # geminate consonant
            elif cur not in self.vowels and idx + 1 < len(sounds) and \
                cur == sounds[idx + 1][0]:
                result += self.geminate

            else:
                kana = self.mora_map.get(cur)
                if kana is None:
                    warnings.warn(f"Unknown mora skipped: {cur}")
                else:
                    result += kana
