class ConsonantConverter:
    ...
    def d_rule(self, word, ph, p_idx):
        # print(f"d_rule: {word}")
        # print(f"p_idx: {p_idx}")
        # print(f"len(ph): {len(ph)}")
        # print(f"ph[p_idx+1]: {ph[p_idx+1]}")
        # d, dz -- dd, z
        if p_idx+1 < len(ph) and ph[p_idx+1] == 'z':
            return 'z'
        # elif p_idx+1 < len(ph) and ph[p_idx+1] == 'ʒ':
        #     #print("hi")
        #     return 'j'
    ...
    def convertConsonant(self, word, ph):
        # ph = ph.replace('dʒ', 'ʤ').replace('ː','').replace('tʃ','ʧ').replace("ɡ","g").replace("ɾ","t")
        # print(f"ph before convertConsonant: {ph}")
        consonant_map = {
            # 'dʒ': self.dg_rule,
            'd': self.d_rule,
            'g': self.gkpt_rule,
